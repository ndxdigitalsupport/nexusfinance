    ...
    <div className="animate-in fade-in duration-200">
      ...
      {showCreate && (
        <form onSubmit={handleCreateOfficer} className="bg-[var(--surface-card)] border border-[var(--border-primary)] rounded-2xl p-6 mb-6 animate-in fade-in slide-in-from-top-2 duration-200">
          <h3 className="font-bold text-[var(--text-primary)] mb-4">{t('new_loan_officer')}</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder={t('full_name')} className="border border-[var(--border-primary)] rounded-lg px-4 py-2.5 text-[14px] focus:outline-none focus:border-[var(--accent)] bg-[var(--surface-primary)] text-[var(--text-primary)]" />
            <input value={newEmail} onChange={e => setNewEmail(e.target.value)} placeholder={t('email_header')} type="email" className="border border-[var(--border-primary)] rounded-lg px-4 py-2.5 text-[14px] focus:outline-none focus:border-[var(--accent)] bg-[var(--surface-primary)] text-[var(--text-primary)]" />
            <input value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder={t('password_placeholder')} type="password" className="border border-[var(--border-primary)] rounded-lg px-4 py-2.5 text-[14px] focus:outline-none focus:border-[var(--accent)] bg-[var(--surface-primary)] text-[var(--text-primary)]" />
          </div>
          <button type="submit" disabled={creating} className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--text-primary)] font-bold text-[14px] px-6 py-2.5 rounded-lg transition cursor-pointer disabled:opacity-50">
            {creating ? t('creating') : t('create_officer')}
          </button>
        </form>
      )}
