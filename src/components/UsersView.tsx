        ...
        <form onSubmit={handleCreateOfficer} className="bg-[var(--surface-card)] border border-[var(--border-primary)] rounded-2xl p-6 mb-6 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <select
              ...
              className="border border-[var(--border-primary)] rounded-lg px-4 py-2.5 text-[14px] focus:outline-none focus:border-[var(--accent)] bg-[var(--surface-primary)] text-[var(--text-primary)] cursor-pointer"
            >
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-[var(--surface-secondary)] text-[11px] uppercase tracking-wider text-[var(--text-secondary)] font-bold sticky top-0">
              <th className="px-5 py-3.5">{t('name_header')}</th>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="relative tenant-dropdown-wrapper">
                        <button
                          ...
                          className="flex items-center gap-1.5 text-[11.5px] font-semibold border border-[var(--border-primary)] rounded-lg px-2.5 py-1.5 bg-[var(--surface-primary)] hover:border-[var(--accent)] transition-all cursor-pointer"
                        >
                          <Building2 className="w-3.5 h-3.5 text-[var(--accent)] shrink-0" />
                          <span className="truncate max-w-[120px]">
                            {tenants.find(t => t.id === u.tenant_id)?.name || `Tenant #${u.tenant_id || 1}`}
                          </span>
                          <ChevronDown className={`w-3 h-3 text-[var(--text-tertiary)] ml-auto transition-transform ${tenantDropdownId === u.id ? 'rotate-180' : ''}`} />
                        </button>
                        {tenantDropdownId === u.id && (
                          <div className="absolute left-0 top-full mt-1 w-52 bg-[var(--surface-card)] border border-[var(--border-primary)] rounded-xl shadow-xl z-30 py-1 max-h-48 overflow-y-auto animate-dropdown-enter">
                            <div className="px-3 py-1 text-[10px] uppercase font-bold text-[var(--text-tertiary)] border-b border-[var(--border-primary)]">
                            ...
                            {tenants.map(t => (
                              <button
                                ...
                                className={`w-full flex items-center justify-between px-3 py-2 text-[12px] text-left cursor-pointer hover:bg-[var(--surface-secondary)] ${t.id === u.tenant_id ? 'font-bold text-[var(--accent)] bg-[var(--accent)]/10' : 'text-[var(--text-primary)]'}`}
                              >
                                <span className="truncate">{t.name}</span>
                                {t.id === u.tenant_id && <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent)]" />}
                              </button>
                            ))}
                          </div>
                        )}
                            ...
                            <div className="absolute left-0 top-full mt-1 w-48 bg-[var(--surface-card)] border border-[var(--border-primary)] rounded-xl shadow-xl shadow-black/5 z-20 py-1.5 overflow-hidden animate-dropdown-enter">
                              {['customer', 'loan-officer', 'admin'].map(r => (
                                ...
                                >
                                  ...
                                  <span className={`${r === u.role ? 'text-[#0d9488]' : 'text-[var(--text-primary)]'}`}>{getRoleLabel(r)}</span>
                                </button>
                              ))}
                            </div>
                          )}
