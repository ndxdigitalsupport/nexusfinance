          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[var(--surface-secondary)] text-[11px] uppercase tracking-wider text-[var(--text-secondary)] font-bold sticky top-0">
                <th className="px-5 py-3.5">{t('id_header')}</th>
                <th className="px-5 py-3.5">{t('amount_header')}</th>
                <th className="px-5 py-3.5">{t('type_header')}</th>
                <th className="px-5 py-3.5">{t('status_header')}</th>
                <th className="px-5 py-3.5">{t('date_header')}</th>
              </tr>
            </thead>
              ...
              {loanPaginated.map((app) => (
                <tr key={app.id} className="text-[14px] font-semibold text-[var(--text-primary)] hover:bg-[var(--surface-secondary)]/70 transition-colors bg-[var(--surface-card)]">
                  ...
                  <td className="px-5 py-3.5">
                    <StatusBadge variant={app.status === 'Approved' ? 'success' : app.status === 'Rejected' ? 'error' : app.status === 'Hold' ? 'warning' : 'info'} label={getStatusLabel(app.status)} />
                  </td>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[18px] font-extrabold text-[var(--text-primary)] tracking-tight">{t('transaction_history')}</h3>
          </div>
        ...
        {transactions.length === 0 ? (
          ...
          <div className="bg-[var(--surface-card)] border border-[var(--border-primary)] rounded-2xl overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[var(--surface-secondary)] text-[11px] uppercase tracking-wider text-[var(--text-secondary)] font-bold sticky top-0">
                  <th className="px-5 py-3.5">{t('title_header')}</th>
                  <th className="px-5 py-3.5">{t('date_header')}</th>
                  <th className="px-5 py-3.5">{t('amount_header')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border-primary)]">
                {txPaginated.map((tx) => (
                  <tr key={tx.id} className="text-[14px] font-semibold text-[var(--text-primary)] hover:bg-[var(--surface-secondary)]/70 transition-colors bg-[var(--surface-card)]">
                    <td className="px-5 py-3.5">{getTxTitle(tx.title)}</td>
